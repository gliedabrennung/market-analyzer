...
только одного писателя на файл разом (NFR-4.1) — перед любой
пишущей командой сервис нужно сперва остановить:
...
До первого `backfill` для символа таблица `klines`/`trades` в DuckDB ещё
не создана — `/ohlcv` и `/analytics/*` в этом случае отвечают сырой 500
(`Catalog Error: Table with name klines does not exist!`), это ожидаемо
и не специфично для Docker. Ещё нюанс именно первого запуска на пустом
`./data/`: `refresh_views` (создание вьюх поверх parquet-файлов)
выполняется только при открытии БД на запись, а на самый первый
`backfill` они ещё не существовали в момент открытия — если `/ohlcv`
всё ещё 500 сразу после первого бэкфилла на свежем каталоге, достаточно
запустить ту же (или любую другую пишущую) команду ещё раз, чтобы
`refresh_views` подхватил уже записанные файлы.
...
(Vite, см. `frontend/README.md`), не читается в рантайме — смена
