...
-- common time grid. Params, in order: interval, then one `?` per symbol
-- (arity fixed up by ma_analytics::correlation at call time — see
-- rowutil::placeholders; only punctuation is spliced in, never data).
WITH rets AS (
    SELECT
        open_time,
        symbol,
    ...
    FROM klines
    WHERE interval = ? AND symbol IN (__SYMBOL_PLACEHOLDERS__)
)
SELECT
